...
from tkinter import *
import tkinter.messagebox # used to build the window
from PIL import ImageTk,Image # python iamging library, used to build images into tkinter the python graphical user interface
import datetime # module to play around with date and time
import mysql.connector # standard database driver for python applications
import MySQLdb # package for containing MySQLdb
conn = mysql.connector.connect(host="localhost",user="root",password="jpwd",database="diahcms")
curs = conn.cursor() # prepare a cursor object using cursor() method

def setBackground(root):
    img = ImageTk.PhotoImage(Image.open(r"O:\dbmslab\10.jpg"))
    label = ttk.Label(root,image = img) # used to display text or image or both
    label.image = img
...
def BSSubWindow(num):
    ...
    BS_categories = {1:'BS_AM',2:'BS_PM',3:'BMI'}
    BS_type = BS_categories[num]
    BS_sub_window.title(BS_type)
    BS_sub_window.geometry("650x470+400+200")
    ...
    bs_amount_entry = ttk.Entry(topFrame, font=('Baskerville',20)) #inputlayout
...
def BSMainWindow():
    BS_main_window = Toplevel() #run the widgit on top
    BS_main_window.title('Blood Sugar')
    BS_main_window.geometry("650x470+400+200")

    setStyle(BS_main_window)
    setBackground(BS_main_window)

    topFrame = Frame(BS_main_window) #geometry master used to provide padding to the toplevel widget
    topFrame.grid(row=3,column=0)
    bottomFrame = Frame(BS_main_window)
    bottomFrame.grid(row=18,column=0)

    BS_AM = ttk.Button(topFrame, text = "BS AM", command=lambda:BSSubWindow(1)) #lambda used for passing parameters
    ...
    BMI = ttk.Button(topFrame, text = "BMI",command=lambda:BSSubWindow(3))
    BMI.grid(row=3,column=1,sticky='NSEW',ipady=10, ipadx=50)
...
def foodSubWindow(num):
    ...
    food_sub_window.geometry("650x470+400+200")
    ...
    def food_submit():
        try:
            if bs_amount_entry.get() == '' or ins_amount_entry.get() == '':
                ...
                args = [ins_amount_entry.get(),bs_amount_entry.get(),(0, 'CHAR')]
                result_args = curs.callproc('insTabAfterMeal', args)

                food_table_entry = 'INSERT INTO MEAL_TAKEN VALUES(%s,%s,%s,%s)'
                input_val = (mealid,bs_amount_entry.get(),result_args[2],1)
...
def foodMainWindow():
    ...
    food_main_window.geometry("650x470+400+200")
...
def insCalcWindow():
    ...
    ins_calc_window.geometry("650x470+400+200")
    ...
    def calDosage():
        take = (85-int(bs_amount_entry.get()))*(0.9375)
        dos_amount_disp.insert(0,take)
...
def insSubWindow():
    ...
    ins_sub_window.geometry("650x470+400+200")
...
def insMainWindow():
    ...
    ins_main_window.geometry("650x470+400+200")
...
def PESubWindow(num):
    ...
    pe_sub_window.geometry("650x470+400+200")
...
def PEMainWindow():
    ...
    pe_main_window.geometry("650x470+400+200")
...
def anomalyWindow():
    ...
    anomaly_window.geometry("650x470+400+200")
...
def driveWindow():
    ...
    drive_window.geometry("650x470+400+200")
    ...
    desc_entry_disp = ttk.Entry(topFrame, font=('Baskerville',20))
    desc_entry_disp.grid(row=4,column=1,sticky='NSWE')

    def driveSubmit():
        if(int(bs_amount_entry.get()) < 80):
            desc_entry_disp.insert(0,'not safe,take insulin')
        else:
            desc_entry_disp.insert(0,'safe to drive')
...
def medWindow():
    ...
    m_main_window.geometry("650x470+400+200")
...
def foodPlanDelWindow():
    ...
    fp3_sub_window.geometry("650x470+400+200")
...
def foodPlanAddWindow():
    ...
    fp2_sub_window.geometry("630x420+400+200")
...
def foodPlanSubWindow():
    ...
    fp_sub_window.geometry("650x470+400+200")
...
def foodPlanMainWindow():
    ...
    fp_main_window.geometry("650x470+400+200")
...
def pePlanDelWindow():
    ...
    pp3_sub_window.geometry("630x420+400+200")
    ...
    get_fp_rows = "SELECT * FROM PE_PLAN"
    curs.execute(get_fp_rows)
    for item in curs:
...
def pePlanAddWindow():
    ...
    pp2_sub_window.geometry("650x470+400+200")
...
def pePlanSubWindow():
    ...
    pp_sub_window.geometry("650x470+400+200")
...
def pePlanMainWindow():
    ...
    pp_main_window.geometry("650x470+400+200")
...
def medPlanDelWindow():
    ...
    mp3_sub_window.geometry("630x420+400+200")
...
def medPlanAddWindow():
    ...
    mp2_sub_window.geometry("630x420+400+200")
...
def medPlanSubWindow():
    ...
    mp_sub_window.geometry("650x470+400+200")
...
def medPlanMainWindow():
    ...
    mp_main_window.geometry("650x470+400+200")
...
def submitMain():
    ...
    udp_med_taken_status = "UPDATE medication set taken_status=1"
    curs.execute(udp_med_taken_status);
...
def main():
    root = Tk() # tkinter works by creating a tcl/tk intrepretor and then translating commands
    root.title("DiaHCMS")
    root.geometry("650x470+400+200") #width and hight

    setStyle(root)
    setBackground(root)

    #BUTTONS FOR DYNAMIC TABLES
    BS = ttk.Button(root,text = "BS & BMI",command=BSMainWindow)
    ...
    FOOD_PLAN.grid(row=10,column=1,sticky = "NSEW")
